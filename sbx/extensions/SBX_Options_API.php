...
class SB_Input {
	...
	public static function text( $args = '' ) {
		...
		$r = wp_parse_args( $args, $defaults );
		extract( $r, EXTR_OVERWRITE );
		$sb_id = SBX::$options_prefix . '[' . esc_attr( $id ) . ']';
		$output = '';
	...
	public static function wysiwyg( $args = '' ) {
		...
		$defaults = array(
			...
			'options'	=> array(   // Options specific to the wp_editor() function
				'textarea_name'	=> SBX::$options_prefix . '[' . esc_attr( $args['id'] ) . ']',
				'media_buttons'	=> false,
				'teeny'			=> true
			)
		);
...
function sb_set_default_options() {
	...
	global $sb_settings_factory;
	$defaults = $current = get_option( SBX::$options_prefix );
	...
	update_option( SBX::$options_prefix, apply_filters( 'sb_option_defaults', $defaults ) );
...
function sb_add_option( $name, $value ) {
	$options = get_option( SBX::$options_prefix );
	if ( $options and !isset($options[$name]) ) {
		$options[$name] = $value;
		return update_option( SBX::$options_prefix, $options );
	} else {
		return false;
	}
}
