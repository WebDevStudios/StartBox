...
function sb_slideshow_verify_post_type() {
	global $post_type;
	
	if (isset( $_GET['post_type'] ) && $_GET['post_type'] == 'slideshow'
		|| isset( $post_type ) && $post_type == 'slideshow') return true; // return true if on a slideshow page
...
function sb_slideshow_embed_input( $post_id ) {
	return '<input class="text urlfield" readonly="readonly" 
		value="[slideshow id=&quot;' . $post_id . '&quot;]" type="text">';
}
...
function sb_slideshow_post_admin_head() {
	...
	if (!sb_slideshow_verify_post_type()) return; // verify post type
?>
	<script type="text/javascript">
		//<![CDATA[
		;(function($) {
			$(document).ready(function(){
				...
				new AjaxUpload($('a#uploadlink'), {
					action: ajaxurl,
					name: fid,
					data: { action: 'sb_action_handle_upload_ajax', _ajax_nonce: '', file_id: fid },
					responseType: 'json',
	...
	<style type="text/css">		
		...
		div.scrollingHotSpotLeft	{
			left: 0;
			background-image: url(<?php echo IMAGES_URL; ?>/admin/arrow_left.gif);
			cursor: url(<?php echo IMAGES_URL; ?>/admin/cursors/cursor_arrow_left.cur), 
				url(images/cursors/cursor_arrow_left.cur), w-resize;
		}
		div.scrollingHotSpotRight {
			right: 0;
			background-image: url(<?php echo IMAGES_URL; ?>/admin/arrow_right.gif);
			cursor: url(<?php echo IMAGES_URL; ?>/admin/cursors/cursor_arrow_right.cur), 
				url(images/cursors/cursor_arrow_right.cur), w-resize;
		}
		...
		#uploaddiv { position:absolute; top:-27px; left:100px; }
